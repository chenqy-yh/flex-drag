...
export const useGuideLine = (config: GuideLineConfig) => {
  ...
  const paintLine = useCallback((axis: "x" | "y", key: number) => {
  ...
  const paintGuidLine = useCallback(
    (e: NodeMoveEvent) => {
      console.log("paintGuidLine",nodes);
      if (nodes.length < 2 || isPainting.current) return;
      // clear line board
